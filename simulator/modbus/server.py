...
async def _updater_loop(
    context: ModbusServerContext, plant: Plant, tick_seconds: float
) -> None:
    """
    Background task: advance the Plant one tick, then write the
    resulting state into the Modbus datastore. Runs forever until
    cancelled.
    ...
    completes atomically with respect to the other.
    """
...
async def run_server(
    ...
    updater_task = asyncio.create_task(_updater_loop(context, plant, tick_seconds))
